(function initializeInMemoryCatalogMetadata(global) {
  ...
  class InMemoryCatalogMetadataStore {
    ...
    openWorkspaceSession(workspaceId) {
      ...
      const replaceCatalogTable = (schemaName, table) => {
        ...
        const operation = record.pending.then(() => {
          ...
          schemas[current.schemaPositions.get(indexKey(schema.metadata.name))] =
            replacementMetadata;
    ...
    listTables(workspaceId, revision, schemaName) {
      const schema = this.#snapshotAtRevision(workspaceId, revision)?.schemas.get(
        indexKey(schemaName),
      );
      return schema
        ? schema.metadata.tables.map((table) => ({
            name: table.name,
    ...
    listViews(workspaceId, revision, schemaName) {
      ...
      return schema
        ? (schema.metadata.views ?? []).map((view) => ({
            name: view.name,
            query: view.query,
          }))
        : [];
  ...
  function normalizeSnapshot(input) {
    ...
    const normalizedSchemas = input.schemas.map((schema, schemaIndexValue) => {
      ...
      const views = rawViews.map((view, viewIndexValue) => {
        const normalized = normalizeView(view, `${path}.views[${viewIndexValue}]`, relationNames);
    ...
    const schemaPositions = new Map(
      normalizedSchemas.map((schema, schemaIndexValue) => [
        indexKey(schema.name),
        schemaIndexValue,
      ]),
    );
  ...
  function normalizeScanner(input, path) {
    ...
    for (const [key, value] of Object.entries(input.options) as [string, any][]) {
      ...
      if (
        expectedType === "string_allow_empty" &&
      ...
      if (
        ...
        invalid(
          `${path}.options.${key} must be a non-empty array of non-empty strings without NUL`,
        );
      }
      if (
        expectedType === "string_or_string_array" &&
