...
const runButton = byId<HTMLButtonElement>("#run-demo");
const resetButton = byId<HTMLButtonElement>("#reset-demo");
const runtimeStatus = byId<HTMLElement>("#runtime-status");
const queryMessage = byId<HTMLElement>("#query-message");
const catalogNameInput = byId<HTMLInputElement>("#catalog-name");
const catalogEditor = byId<HTMLTextAreaElement>("#catalog-editor");
const sqlEditor = byId<HTMLTextAreaElement>("#sql-editor");
const resultHead = byId<HTMLTableSectionElement>("#result-head");
const resultBody = byId<HTMLTableSectionElement>("#result-body");
const resultSummary = byId<HTMLElement>("#result-summary");
const fixtureUrlValue = byId<HTMLAnchorElement>("#fixture-url");
const fixtureContentVersionValue = byId<HTMLElement>("#fixture-content-version");
const fixtureRowsValue = byId<HTMLElement>("#fixture-rows");
const fixtureBytesValue = byId<HTMLElement>("#fixture-bytes");
const fixtureColumnsValue = byId<HTMLElement>("#fixture-columns");
const logOutput = byId<HTMLElement>("#diagnostics");
const steps = new Map(
  [...document.querySelectorAll<HTMLElement>("[data-step]")].map((element) => [
...
async function loadDuckDB() {
  duckdb ??= await import(/* @vite-ignore */ new URL("./duckdb/duckdb-browser.mjs", rootUrl).href);
  return duckdb;
}
...
function populateFixtureInfo(metadata, fileUrl) {
  ...
  fixtureColumnsValue.textContent = metadata.columns
...
async function resetEditors({ announce = true } = {}) {
  ...
  if (announce) {
    ...
    setStep(
      "query",
      activeRuntime ? "idle" : "error",
      activeRuntime ? "Ready" : "Runtime unavailable",
    );
  }
}
...
async function startRuntime() {
  ...
  try {
    ...
    worker = createInMemoryCatalogWorker({
      ...
      workerUrl: new URL("./in-memory-catalog/in-memory-catalog-worker.js", rootUrl),
    });
    database = new duckdbModule.AsyncDuckDB(new duckdbModule.VoidLogger(), worker);
