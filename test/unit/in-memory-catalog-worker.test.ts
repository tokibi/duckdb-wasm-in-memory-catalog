...
const workerSource = await readFile(
  new URL("../../src/javascript/in-memory-catalog-worker.ts", import.meta.url),
  "utf8",
);
...
describe("In-Memory Catalog Worker entrypoint", () => {
  it("dispatches catalog messages inline and preserves the DuckDB worker context", async () => {
    ...
    assert.deepEqual(
      handled.map(({ kind, event }) => ({ kind, event })),
      [
    ...
    assert.equal(handled[1].context.DUCKDB_IN_MEMORY_CATALOG, context.DUCKDB_IN_MEMORY_CATALOG);
