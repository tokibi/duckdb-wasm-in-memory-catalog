...
- [**Getting Started**](./getting-started.md): ランタイムのセットアップ、アセットの配置、初期化とクエリ実行手順。
- [**Guides**](./guides.md): カタログの公開、テーブルやビューの更新、クエリの実行、クリーンアップなどのレシピ。
- [**Concepts**](./concepts.md): 完全スナップショット、テーブルスナップショット、キャッシュ分離の仕組み、読み取り専用である理由。
- [**Scanners**](./scanners.md): Parquet、CSV、JSON、XLSX の各スキャナの設定方法と利用可能なオプション一覧。
- [**Reference**](./reference.md): スナップショットの JSON スキーマ定義、JavaScript API の詳細、エラーハンドリング、制約事項。
